export type Lote = { descricao: string; quantidade: number; area_m2: number; valor_m2: number };
...
export type FluxoMes = {
  ...
  receita: number;
  despesa: number;
  /** compat: total de saídas do mês */
  saida: number;
  /** compat: total de entradas do mês */
  entrada: number;
  liquido: number;
  acumulado: number;
};
...
/** VGV a partir da tabela de lotes (quantidade x área x valor por m²). */
export function vgvDosLotes(lotes: Lote[]): { vgv: number; area: number } {
  let vgv = 0;
  let area = 0;
...
/** Net present value of a monthly cash-flow series (index 0 = month 0). */
export function npv(rate: number, flows: number[]): number {
  return flows.reduce((acc, f, i) => acc + f / Math.pow(1 + rate, i), 0);
}

/** IRR by bisection over a monthly cash-flow series. Returns null when it does not converge. */
export function irr(flows: number[]): number | null {
  ...
  // Amplia o limite superior até haver troca de sinal (fluxos muito rentáveis).
  let tentativas = 0;
  ...
  // Fallback: varredura fina para localizar o intervalo com troca de sinal.
...
export function calcViabilidade(i: ViabilidadeInput): ViabilidadeResult {
  ...
  const vendaMes = i.vgv / nVendas; // valor vendido a cada mês de comercialização

  // Recebimentos: cada venda do mês v gera entrada em v e parcelas de v+1..v+nParcelas
  ...
  for (let m = 1; m <= horizonte; m++) {
    ...
    // comissão é paga na assinatura da venda, não no recebimento das parcelas
    const comissaoMes =
  ...
  const payback = fluxo.find((f) => f.acumulado >= 0 && f.mes > 0)?.mes ?? null;
  const pior = fluxo.reduce((a, b) => (b.acumulado < a.acumulado ? b : a), fluxo[0]);
...
export const pctBR = (n: number | null | undefined, casas = 2) =>
  n == null || !isFinite(n) ? "—" : `${n.toLocaleString("pt-BR", { minimumFractionDigits: casas, maximumFractionDigits: casas })}%`;
...
export const brlShort = (n: number) => {
  const a = Math.abs(n);
  const s = n < 0 ? "-" : "";
  if (a >= 1_000_000) return `${s}R$ ${(a / 1_000_000).toLocaleString("pt-BR", { maximumFractionDigits: a >= 10_000_000 ? 0 : 1 })}M`;
