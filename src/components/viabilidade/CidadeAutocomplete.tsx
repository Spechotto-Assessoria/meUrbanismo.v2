...
export function buscarCidades(termo: string, lista: Cidade[]): Cidade[] {
    if (!termo) return lista.slice(0, 50);
    ...
    if (!pending) {
        ...
        pending = fetch("https://servicosdados.ibge.gov.br/api/v1/localidades/municipios?orderBy=nome")
            ...
            .then((rows: any[]) =>
                rows.map((m) => ({
                    nome: String(m.nome),
                    uf: String(m.microrregiao?.mesorregiao?.UF?.sigla ?? m["regiao-imediata"]?.["regiao-intermediaria"]?.UF?.sigla ?? ""),
                }))
            )
            .then((list) => {
                cacheIbge = list;
...
export function CidadeAutocomplete({ value, onChange }: { value: string; onChange: (v: string) => void; }) {
    ...
    return (
        <div className="relative w-full" ref={containerRef}>
            <Button type="button" variant="outline" onClick={() => setOpen(!open)} className="w-full justify-between font-normal bg-slate-50 border-slate-200 rounded-xl">
                <span className={cn("truncate", !value && "text-slate-500")}>{value || "Buscar cidade do Brasil..."}</span>
            ...
            {open && (
                <div className="absolute top-full left-0 z-50 mt-1 w-full rounded-xl border border-slate-200 bg-white shadow-lg overflow-hidden">
                    <div className="border-b border-slate-100 p-2 bg-slate-50">
                        <Input autoFocus value={termo} onChange={(e: any) => setTermo(e.target.value)} placeholder="Digite a cidade..." className="h-9 bg-white" />
                    </div>
                    <div className="max-h-64 overflow-y-auto p-1">
                        {resultados.map((m) => {
                            const label = m.uf ? `${m.nome} - ${m.uf}` : m.nome;
                            return (
                                <button key={`${m.nome}-${m.uf}`} type="button" onClick={() => { onChange(label); setOpen(false); setTermo(""); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm hover:bg-slate-100 transition-colors cursor-pointer">
                        ...
                        {digitado.length > 0 && (
                            <button type="button" onClick={() => { onChange(digitado); setOpen(false); setTermo(""); }} className="mt-1 w-full rounded-lg border border-dashed border-purple-200 px-3 py-2.5 text-left text-xs font-medium text-purple-700 bg-purple-50 hover:bg-purple-100 transition-colors cursor-pointer">
                                Usar “{digitado}”
                            </button>
                        )}
                    </div>
