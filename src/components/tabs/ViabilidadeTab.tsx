...
import { brlCents, rotuloPayback, pctBR } from '../../lib/viabilidade';
import { diagnosticarViabilidade } from '../../lib/viabilidade-diagnostico';
import { AuditoriaPremissasViabilidade } from '../viabilidade/AuditoriaPremissasViabilidade';
import { InsightsViabilidade } from '../viabilidade/InsightsViabilidade';
...
const ViabilidadeConteudo: React.FC<{ canEdit: boolean }> = ({ canEdit }) => {
  ...
  const diagnostico = useMemo(
    () =>
      diagnosticarViabilidade({
        input: { ...v.form, vgv: v.vgv, custo_obra: v.orcamento.custoObra },
        resultado: r,
        precoM2: v.precoEfetivoM2,
        areaM2: v.fonteVgv === 'lotes' ? v.areaLotes : v.areaVendavel,
        fonteVgv: v.fonteVgv,
      }),
    [v.form, v.vgv, v.orcamento.custoObra, r, v.precoEfetivoM2, v.fonteVgv, v.areaLotes, v.areaVendavel]
  );

  if (v.loading) return <SkeletonTable />;

  const payback = rotuloPayback(r.paybackDescontadoMeses, r.paybackMeses);
  ...
  const kpis = [
    ...
    { label: 'Payback', value: payback.value, hint: payback.hint, icon: Clock, tone: 'text-orange-700', iconTone: 'text-orange-600', help: AJUDA_KPI.payback },
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
        <div>
          ...
          <h1 className="text-xl font-black text-slate-900 mt-1 flex items-center gap-2">
            <PieChart className="w-5 h-5 text-navy-800 shrink-0" /> Viabilidade Econômico-Financeira
          </h1>
        ...
        <div className="flex flex-wrap items-stretch gap-2">
          <div className="inline-flex h-10 items-center gap-2 bg-slate-50 px-2.5 rounded-xl border border-slate-200 text-xs shadow-sm">
            <label className="text-[9px] font-bold text-slate-500 uppercase whitespace-nowrap">R$/m² Médio</label>
            <input
              type="number"
              ...
              className="h-7 w-24 px-2 rounded-lg bg-white border border-slate-200 text-xs font-bold text-slate-800 text-right disabled:opacity-60 focus:outline-none focus:ring-1 focus:ring-navy-800"
            />
          </div>
          ...
          {canEdit && (
            <button
              ...
              className="inline-flex h-10 items-center gap-1.5 px-3.5 rounded-xl bg-navy-900 hover:bg-slate-800 text-xs font-semibold text-white disabled:opacity-50 shadow-sm"
            >
